...
def simple_load_balancer(args):
    num_clients = 10
    num_servers = 10
    delays=[1, 1, 1, 2, 1, 2, 3, 1, 5, 1]
    weights=[3, 3, 3, 2, 3, 2, 3, 2, 1, 3]
    ...
    info( '*** Adding clients\n' )
    clients = [net.addHost(f'h{i}',) for i in range(1, num_clients + 1)]
    info( '*** Adding servers\n' )
    servers = [net.addHost(f'h{i}',) for i in range(num_clients + 1, num_clients + num_servers + 1)]
    info( '*** Adding load balancer\n' )
    lb = net.addHost(f'h{num_clients + num_servers + 1}', ip='10.10.10.10')
    ...
    for (i, server) in enumerate(servers):
        server.cmdPrint(f'python ./mininet/server.py -l 0.0.0.0 -p 80 -d {delays[i]} {server.name} &')
